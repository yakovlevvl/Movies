...
extension URL {
    ...
    func createVideoThumbnail() -> UIImage? {
        return AVAsset(url: self).createThumbnailOfVideo()
    }
    
    func createGifThumbnail() -> UIImage? {
        let options = [kCGImageSourceShouldCache: kCFBooleanFalse] as CFDictionary
        guard let imageSource = CGImageSourceCreateWithURL(self as CFURL, options) else {
            return nil
        }
    ...
    func getRtfText(with font: UIFont) -> NSMutableAttributedString? {
        guard let attributedString = try? NSMutableAttributedString(url: self, options: [.documentType : NSAttributedString.DocumentType.rtf], documentAttributes: nil) else { return nil }
...
extension AVAsset {
    ...
    static func compressVideo(inputURL: URL, outputURL: URL, completion: @escaping () -> ()) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let writerSettings: [String : Any] = [AVVideoCodecKey : AVVideoCodecType.h264, AVVideoCompressionPropertiesKey : [AVVideoAverageBitRateKey : 1000000], AVVideoWidthKey : 1280, AVVideoHeightKey : 720] // 1800000
    ...
    static func compressFile(inputURL: URL, outputURL: URL, completion: @escaping (URL) -> ()){
    ...
    static func checkFileSize(sizeUrl: URL, message:String){
...
extension UIImage {
    ...
    func resizeAsync(to size: CGSize, completion: @escaping (UIImage) -> ()) {
        ...
        guard width < self.size.width, height < self.size.height else {
...
extension Date {
    ...
    func timeAgoDisplay(shortVersion: Bool = false) -> String {
        ...
        if minuteAgo < self {
            ...
            return shortVersion ? "\(diff) sec" : "\(abs(diff)) \(abs(diff) == 1 ? "second" : "seconds") ago"
        } else if hourAgo < self {
            let diff = Calendar.current.dateComponents([.minute], from: self, to: Date()).minute ?? 0
            return shortVersion ? "\(diff) min" : "\(abs(diff)) \(abs(diff) == 1 ? "minute" : "minutes") ago"
        } else if dayAgo < self {
            let diff = Calendar.current.dateComponents([.hour], from: self, to: Date()).hour ?? 0
            return shortVersion ? "\(diff) hours" : "\(abs(diff)) \(abs(diff) == 1 ? "hour" : "hours") ago"
        } else if weekAgo < self {
            let diff = Calendar.current.dateComponents([.day], from: self, to: Date()).day ?? 0
            return shortVersion ? "\(diff) days" : "\(abs(diff)) \(abs(diff) == 1 ? "day" : "days") ago"
        }
        let diff = Calendar.current.dateComponents([.weekOfYear], from: self, to: Date()).weekOfYear ?? 0
        return shortVersion ? "\(diff) weeks" : "\(abs(diff)) \(abs(diff) == 1 ? "week" : "weeks") ago"
    }
}
